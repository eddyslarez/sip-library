...
/**
 * Builder for creating different types of SIP messages - Enhanced version
 * Versión mejorada con soporte para headers personalizados y configuración avanzada
 *
 * @author Eddys Larez
 */
object SipMessageBuilder {
    ...
    /**
     * Build REGISTER message with enhanced configuration support
     */
    fun buildRegisterMessage(
        ...
        customExpires: Int? = null
    ): String {
        val uri = "sip:${accountInfo.domain}"
        val builder = StringBuilder()
        val expires = customExpires ?: accountInfo.expirationSeconds
        ...
        builder.append("User-Agent: ${accountInfo.userAgent ?: "EddysSipLibrary/1.0"}\r\n")

        // Contact header con parámetros dinámicos
        builder.append("Contact: ${accountInfo.buildContactHeader(isAppInBackground)};expires=$expires\r\n")
        ...
        accountInfo.getCustomSipHeaders().forEach { (name, value) ->
            if (name !in listOf("Via", "From", "To", "Call-ID", "CSeq", "User-Agent", "Contact", "Expires", "Authorization")) {
    ...
    fun buildUnregisterMessage(
        ...
        return buildRegisterMessage(
            ...
            customExpires = UNREGISTER_EXPIRES
        )
    }

    /**
     * Build INVITE message with enhanced features
     */
    fun buildInviteMessage(
        ...
        // Headers personalizados del callData
        callData.customHeaders.forEach { (name, value) ->
            if (name !in listOf("Via", "From", "To", "Call-ID", "CSeq", "Contact", "User-Agent", "Authorization", "Content-Type", "Content-Length")) {
                builder.append("$name: $value\r\n")
            }
        }

        // Headers personalizados de la cuenta
        accountInfo.getCustomSipHeaders().forEach { (name, value) ->
            if (name !in listOf("Via", "From", "To", "Call-ID", "CSeq", "Contact", "User-Agent", "Authorization", "Content-Type", "Content-Length")) {
                builder.append("$name: $value\r\n")
            }
    ...
    /**
     * Build call control messages with enhanced features
     */
    private fun buildCallControlMessage(
        ...
        // Headers personalizados si están habilitados
        if (includeCustomHeaders) {
            callData.customHeaders.forEach { (name, value) ->
                if (name !in listOf("Via", "From", "To", "Call-ID", "CSeq", "User-Agent", "Content-Length")) {
                    builder.append("$name: $value\r\n")
                }
            }
    ...
    /**
     * Build ACK message with enhanced features
     */
    fun buildAckMessage(accountInfo: AccountInfo, callData: CallData): String {
        ...
        return buildString {
            ...
            append("User-Agent: ${accountInfo.userAgent ?: "EddysSipLibrary/1.0"}\r\n")
    ...
    /**
     * Build SIP responses with enhanced features
     */
    private fun buildSipResponse(
        ...
        return buildString {
            ...
            if (includeCustomHeaders) {
                callData.customHeaders.forEach { (name, value) ->
                    if (name !in listOf("Via", "From", "To", "Call-ID", "CSeq", "User-Agent", "Contact", "Content-Type", "Content-Length")) {
                        append("$name: $value\r\n")
                    }
                }
    ...
    // Response builders using the enhanced common function
    ...
    private fun buildGenericOkResponse(lines: List<String>): String {
        ...
        return buildString {
            ...
            append("User-Agent: EddysSipLibrary/1.0\r\n")
            append("Content-Length: 0\r\n\r\n")
        }
    }
    ...
    /**
     * Build re-INVITE message with enhanced features
     */
    fun buildReInviteMessage(
        ...
        return buildString {
            ...
            // Headers personalizados
            callData.customHeaders.forEach { (name, value) ->
                if (name !in listOf("Via", "From", "To", "Call-ID", "CSeq", "Contact", "User-Agent", "Content-Type", "Content-Length")) {
                    append("$name: $value\r\n")
                }
            }
    ...
    /**
     * Build INFO message for DTMF with enhanced features
     */
    fun buildDtmfInfoMessage(
        ...
        try {
            ...
            log.d(tag = "SipMessageBuilder") { "Enhanced DTMF INFO message built for digit '$normalizedDigit'" }
            return message
