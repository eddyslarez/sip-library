...
/**
 * Handles SIP message processing and generation - Enhanced version
 * Versión mejorada con mejor manejo de eventos y callbacks
 *
 * @author Eddys Larez
 */
class SipMessageHandler(private val sipCoreManager: SipCoreManager) {
    ...
    fun handleSipMessage(message: String, accountInfo: AccountInfo) {
        try {
            ...
            notifyError("SIP message handling error: ${e.message}", e)
        }
    }
    ...
    /**
     * Handle SIP requests with enhanced routing
     */
    private fun handleSipRequest(
        ...
        when (method) {
            ...
            "INFO" -> handleIncomingInfo(message, accountInfo, lines)
        }
    }

    /**
     * Handle SIP responses with enhanced routing
     */
    fun handleSipResponse(
        ...
        when (method) {
            ...
            "INFO" -> handleInfoResponse(statusCode, message, accountInfo, lines)
    ...
    private fun handleIncomingInvite(message: String, accountInfo: AccountInfo, lines: List<String>) {
        ...
        try {
            ...
            notifyError("Error handling incoming call: ${e.message}", e)
        }
    }
    ...
    private fun createIncomingCallData(message: String, lines: List<String>, accountInfo: AccountInfo): CallData {
        ...
        val customHeaders = extractCustomHeaders(lines)
    ...
    private fun extractCustomHeaders(lines: List<String>): Map<String, String> {
        val customHeaders = mutableMapOf<String, String>()
        val standardHeaders = setOf(
            "via", "from", "to", "call-id", "cseq", "contact", "content-type",
            "content-length", "user-agent", "max-forwards", "expires", "authorization"
        )

        lines.forEach { line ->
            if (line.contains(":")) {
                val headerName = line.substringBefore(":").trim().lowercase()
                if (headerName !in standardHeaders) {
    ...
    private fun updateUIForIncomingCall(callData: CallData) {
        ...
        notifyCallStateChanged(CallState.INCOMING, callData.callId)
        ...
        try {
            ...
            when (requestType) {
                "BYE" -> {
                    ...
                    val duration = if (sipCoreManager.callStartTimeMillis > 0) {
                        endTime - sipCoreManager.callStartTimeMillis
                    } else 0
                    ...
                    notifyCallDisconnected(callId, "Call terminated by remote party", duration)
                }
                "CANCEL" -> {
                    ...
                    // Notificar desconexión
                    notifyCallDisconnected(callId, "Call cancelled", 0)
                }
            }

            // Common termination cleanup
            CallStateManager.updateCallState(CallState.ENDED)
            notifyCallStateChanged(CallState.ENDED, callId)
            ...
            notifyError("Error handling $requestType: ${e.message}", e)
        }
    }
    ...
    private fun handleIncomingAck(message: String, accountInfo: AccountInfo, lines: List<String>) {
        ...
        try {
            ...
            if (sipCoreManager.callState == CallState.ACCEPTING) {
                ...
                notifyCallStateChanged(CallState.CONNECTED, callId)
                notifyCallConnected(callId)
            ...
            notifyError("Error handling ACK: ${e.message}", e)
        }
    }

    private fun handleIncomingInfo(message: String, accountInfo: AccountInfo, lines: List<String>) {
        log.d(tag = TAG) { "📋 INFO received" }
        ...
        try {
            ...
            if (contentType.contains("application/dtmf-relay", ignoreCase = true)) {
                val dtmfContent = SipMessageParser.extractSdpContent(message)
                val signalMatch = Regex("Signal=([0-9A-D*#])").find(dtmfContent)

                signalMatch?.let { match ->
                    val digit = match.groupValues[1].first()
                    log.d(tag = TAG) { "DTMF received: $digit" }
                    notifyDtmfReceived(digit)
                }
            }

            // Send 200 OK response for INFO
            val okResponse = SipMessageBuilder.buildGenericOkResponse(lines)
            accountInfo.webSocketClient?.send(okResponse)

        } catch (e: Exception) {
            log.e(tag = TAG) { "Error handling incoming INFO: ${e.stackTraceToString()}" }
        }
    }
    ...
    private fun handleReInviteResponse(statusCode: Int?, message: String, accountInfo: AccountInfo, lines: List<String>) {
        ...
        when (statusCode) {
            ...
            CODE_OK -> {
                ...
                // Notificar evento de hold/resume
                if (callData.isOnHold == true) {
                    notifyCallHeld(callData.callId)
                } else {
                    notifyCallResumed(callData.callId)
                }
            }
            CODE_UNAUTHORIZED -> handleAuthenticationChallenge(accountInfo, message, lines)
            CODE_BUSY -> {
    ...
    private fun restorePreviousHoldState(callData: CallData) {
        ...
        notifyCallStateChanged(newState, callData.callId)
    }
    ...
    private fun handleInfoResponse(statusCode: Int?, message: String, accountInfo: AccountInfo, lines: List<String>) {
        when (statusCode) {
            CODE_OK -> {
                log.d(tag = TAG) { "INFO request accepted (200 OK)" }
            }
            else -> {
                log.d(tag = TAG) { "INFO request failed: $statusCode" }
            }
        }
    }
    ...
    private fun handleInviteOk(message: String, accountInfo: AccountInfo, lines: List<String>) {
        ...
        sendAck(accountInfo, callData)
        CallStateManager.updateCallState(CallState.CONNECTED)
        notifyCallStateChanged(CallState.CONNECTED, callData.callId)
        notifyCallConnected(callData.callId)
        sipCoreManager.callStartTimeMillis = Clock.System.now().toEpochMilliseconds()
    }
    ...
    private fun handleRequestTerminated(accountInfo: AccountInfo, lines: List<String>) {
        ...
        if (callData != null) {
            ...
            notifyCallDisconnected(callData.callId, "Call cancelled", 0)
        }

        CallStateManager.updateCallState(CallState.ENDED)
        notifyCallStateChanged(CallState.ENDED, callData?.callId ?: "")
        accountInfo.resetCallState()
    }
    ...
    private fun handleTrying() {
        ...
        notifyCallStateChanged(CallState.OUTGOING, "")
    }

    private fun handleRinging() {
        sipCoreManager.playRingtoneUseCase.playOutgoingRingtone()
        log.d(tag = TAG) { "Ringing/Session Progress (180/183)" }
        CallStateManager.updateCallState(CallState.RINGING)
        notifyCallStateChanged(CallState.RINGING, "")
    }
    ...
    private fun handleRegisterOk(message: String, accountInfo: AccountInfo, lines: List<String>) {
        ...
        notifyRegistrationSuccess(accountInfo.getAccountIdentity(), expires)
    ...
    private fun handleBusy() {
        ...
        callData?.let {
            notifyCallFailed(it.callId, "Busy")
        }
    }
    ...
    private fun handleOtherStatusCodes(statusCode: Int?) {
        ...
        if (statusCode != null && statusCode >= 400) {
            ...
            notifyRegistrationFailed("", "Registration failed with code $statusCode")

            val callData = sipCoreManager.currentAccountInfo?.currentCallData
            callData?.let {
                notifyCallFailed(it.callId, "Call failed with code $statusCode")
            }
        }
    }
    ...
    private fun handleAuthenticationChallenge(accountInfo: AccountInfo, message: String, lines: List<String>) {
        ...
        try {
            ...
            notifyError("Authentication error: ${e.message}", e)
        }
    }

    // ===================== NOTIFICATION METHODS =====================

    private fun notifyRegistrationSuccess(account: String, expiresIn: Int) {
        sipCoreManager.eventListener?.onRegistrationSuccess(account, expiresIn)
    }

    private fun notifyRegistrationFailed(account: String, error: String) {
        sipCoreManager.eventListener?.onRegistrationFailed(account, error)
    }

    private fun notifyIncomingCall(callData: CallData) {
        sipCoreManager.eventListener?.onIncomingCall(
            callData.from,
            callData.remoteDisplayName.takeIf { it.isNotEmpty() },
            callData.callId
        )
    }

    private fun notifyCallStateChanged(state: CallState, callId: String) {
        sipCoreManager.eventListener?.onCallStateChanged(state, callId)
    }

    private fun notifyCallConnected(callId: String) {
        sipCoreManager.eventListener?.onCallConnected(callId, 0)
    }

    private fun notifyCallDisconnected(callId: String, reason: String, duration: Long) {
        sipCoreManager.eventListener?.onCallDisconnected(callId, reason, duration)
    }

    private fun notifyCallFailed(callId: String, error: String) {
        sipCoreManager.eventListener?.onCallFailed(callId, error)
    }

    private fun notifyCallHeld(callId: String) {
        sipCoreManager.eventListener?.onCallHeld(callId)
    }

    private fun notifyCallResumed(callId: String) {
        sipCoreManager.eventListener?.onCallResumed(callId)
    }

    private fun notifyDtmfReceived(digit: Char) {
        sipCoreManager.eventListener?.onDtmfReceived(digit)
    }

    private fun notifyError(error: String, exception: Throwable?) {
        sipCoreManager.eventListener?.onLibraryError(error, exception)
    }
    ...
    private fun sendSipMessage(messageBuilder: () -> String, messageType: String, accountInfo: AccountInfo) {
        try {
            ...
            notifyError("Error sending $messageType: ${e.message}", e)
        }
    }
