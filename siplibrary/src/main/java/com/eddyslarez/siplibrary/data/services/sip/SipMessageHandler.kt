...
class SipMessageHandler(
    ...
    private fun handleIncomingInvite(message: String, accountInfo: AccountInfo, lines: List<String>) {
        log.d(tag = TAG) { "🔔 Incoming call received" }
        sipCoreManager.setCurrentAccountInfo(accountInfo)
        sipCoreManager.setCallFromPush(true)
    ...
    private fun updateUIForIncomingCall(callData: CallData) {
        StateManager.updateCallerNumber(callData.from)
        StateManager.updateCallId(callData.callId)
        StateManager.updateCallState(CallState.INCOMING)

        CoroutineScope(Dispatchers.IO).launch {
            sipCoreManager.getRingtoneManager().playIncomingRingtone()
        }

        sipCoreManager.bringToFront()
    }
        ...
        try {
            ...
            when (requestType) {
                "BYE" -> {
                    ...
                    // Si la llamada estaba conectada, fue exitosa, sino fue perdida
                    val endTime = Clock.System.now().toEpochMilliseconds()
                    val callType = if (StateManager.getCurrentCallState() == CallState.CONNECTED) {
                        CallTypes.SUCCESS
                    } else if (currentCallData.direction == CallDirections.INCOMING) {
                        CallTypes.MISSED
                    } else {
    ...
    private fun handleIncomingAck(message: String, accountInfo: AccountInfo, lines: List<String>) {
        ...
        try {
            ...
            if (StateManager.getCurrentCallState() == CallState.ACCEPTING) {
                StateManager.updateCallState(CallState.CONNECTED)
                log.d(tag = TAG) { "🟢 Call connected after receiving ACK" }
                sipCoreManager.setCallStartTime(Clock.System.now().toEpochMilliseconds())
                accountInfo.isCallConnected = true
                accountInfo.callStartTime = sipCoreManager.getCallStartTime()
            }
        } catch (e: Exception) {
            log.e(tag = TAG) { "Error handling incoming ACK: ${e.stackTraceToString()}" }
        }
    ...
    private fun handleTrying() {
        log.d(tag = TAG) { "Trying (100)" }
        StateManager.updateCallState(CallState.OUTGOING)
    }

    private fun handleRinging() {
        sipCoreManager.getRingtoneManager().playOutgoingRingtone()
        log.d(tag = TAG) { "Ringing/Session Progress (180/183)" }
    }
    ...
    private fun handleRegisterOk(message: String, accountInfo: AccountInfo, lines: List<String>) {
        ...
        accountInfo.isRegistered = true
        StateManager.updateRegistrationState(RegistrationState.OK)
    ...
    private fun handleOtherStatusCodes(statusCode: Int?) {
        ...
        if (statusCode != null && statusCode >= 400) {
            ...
            StateManager.updateCallState(CallState.ERROR)
        }
    }
    ...
    private fun sendAuthenticatedRegister(accountInfo: AccountInfo) {
        ...
        sendSipMessage(
            messageBuilder = { SipMessageBuilder.buildAuthenticatedRegisterMessage(accountInfo, StateManager.getCurrentAppState() == EddysSipLibrary.AppState.BACKGROUND) },
            messageType = "authenticated REGISTER",
            accountInfo = accountInfo
        )
    }
